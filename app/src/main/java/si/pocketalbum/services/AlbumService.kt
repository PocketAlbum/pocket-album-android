...
class AlbumService : Service() {
    ...
    private val connectionFlow = MutableStateFlow<Deferred<AlbumConnection>?>(null)
    val openedAlbum: StateFlow<Deferred<AlbumConnection>?> = connectionFlow
    ...
    fun loadAlbumAsync(locator: AlbumLocator? = null) {
        connectionFlow.value = CoroutineScope(Job() + Dispatchers.IO).async {
            loadAlbum(locator)
        }
    }
    ...
    override fun onDestroy() {
        ...
        try {
            connectionFlow.value?.getCompleted()?.close();
        }
        catch (e: Exception) {
    ...
    fun getHeatmapCache(): HeatmapCache {
        try {
            return connectionFlow.value!!.getCompleted().heatmaps
        }
        catch (e: Exception) {
